...
module.exports.visit = function visit(configOptions, getValue) {
  var result = {};
  return visitConfig(result, {object: configOptions, path: ''}, getValue);
};
...
function visitConfig(result, objectValueAndPath, getValue) {
  var object = objectValueAndPath.object;
  var path = objectValueAndPath.path;
  var keys = Object.keys(object);
  u.each(keys, function forEachKey(key) {
    var toVisit = object[key];
    ...
    if (undefined !== getValue && null !== getValue && isValue(toVisit)) {
      ...
      result[key] = {};
      result[key] = visitConfig(result[key], {object: toVisit, path: pathToElement}, getValue);
    }
  });
  return result;
}
