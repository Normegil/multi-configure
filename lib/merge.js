...
module.exports = function merge(sources, allResults, callback) {
  sortResults(allResults, sources, function onSorted(err, sortedResults) {
    ...
    var results = u.map(sortedResults, function getConfig(result) {
...
function sortResults(results, sources, callback) {
  var sourcesWithPriority = u.filter(sources, function filterWithPriority(source) {
    return h.exist(source.priority);
  });

  var sourcesWithoutPriority = u.filter(sources, function filterWithPriority(source) {
    return !h.exist(source.priority);
  });

  var sortedSourcesWithPriority = u.sortBy(sourcesWithPriority, function sortSources(source) {
    return -source.priority;
  });

  var prioritizedSources = u.union(sortedSourcesWithPriority, sourcesWithoutPriority);

  var sourceWithInternalPriority =
    u.map(prioritizedSources, function addInternalPriority(source, index) {
      source.internalPriority = index;
      return source;
    });

  var resultsWithMetaData = u.sortBy(results, function sortBy(result) {
    var source = u.filter(sourceWithInternalPriority, function filterSources(source) {
