...
module.exports = {
  ...
  load: function load(options, log) {
    return new Promise(function load(resolve, reject) {
      ...
      log.debug({arguments: args, structure: structure}, 'Command Line: Load command line options');
      h.visit(structure, '', {
        ...
        log.trace({config: config}, 'Command Line: Structure visited');
...
function enterObject(options, log) {
  return new Promise(function enterObject(resolve, reject) {
    ...
    log.debug({arguments: options.args, options: cmdOpts},
      'Command Line: get values for arguments');
    let values = getCommandLineValues(options.args, cmdOpts);
    log.debug({arguments: options.args, options: cmdOpts, values: values},
      'Command Line: values loaded');
